...
**Критичних:** 3 (Проблема #3/#5 - одна проблема, #6 - критична для UE 5.6)
**Середньої важливості:** 3

**Загальний висновок:**
MCP плагін використовує **застарілий API (UE 4.x)** для роботи з компонентами, що призводить до некоректної роботи в **UE 5.6**. Компоненти створюються але не інтегруються з Editor UI через відсутність використання USubobjectDataSubsystem.

**Критичні проблеми:**
1. **Problem #6**: Використання SimpleConstructionScript API замість USubobjectDataSubsystem - компоненти не видимі в Editor
2. **Problem #3/#5**: create_node_by_action_name ігнорує class_name при виборі функції
3. **Problem #1**: ✅ RESOLVED - WidgetComponent тепер підтримується

**Рекомендації (КРИТИЧНО):**
1. **НЕГАЙНО**: Мігрувати ComponentService на USubobjectDataSubsystem API для UE 5.6
2. Виправити баг в create_node_by_action_name для правильного розрізнення функцій
...
**Статус:** CRITICAL - блокує правильне додавання компонентів в UE 5.6

**Impact:** 
- Компоненти створюються але невидимі в Editor UI
- Неможливо налаштувати властивості через Details panel
- Компоненти не з'єднані з правильним батьком (Capsule для Character)
- Порушує workflow розробника
