"""Generate 3 Cyclosorus-style fern variants based on proven clean coefficients.
All variants based on the working 03 pattern with safe variations.
Includes automatic quality validation."""
...
# All variants based on 03 (the one that works cleanly)
# Varying: pinnae count (d in t2), width (coefficients in t3/t4), overall shape
VARIANTS = [
    {
        "name": "T_Fern_Cyclosorus_01",
        "seed": 33,
        # The proven clean one — straight, wide pinnae, ромбоподібний
        "transforms": [
            (0.0, 0.0, 0.0, 0.25, 0.0, -0.14, 0.02),
            (0.94, 0.0, 0.0, 0.91, 0.0, 0.55, 0.84),
            (0.05, -0.22, 0.20, 0.055, -0.08, 0.02, 0.07),
            (-0.05, 0.22, 0.20, 0.055, 0.08, 0.12, 0.07),
        ],
    },
    {
        "name": "T_Fern_Cyclosorus_02",
        "seed": 44,
        # Fewer but longer pinnae — more spacing between them
        # Lower d in t2 = more vertical compression = fewer pinnae rows
        # Larger coefficients in t3/t4 = longer pinnae
        "transforms": [
            (0.0, 0.0, 0.0, 0.25, 0.0, -0.14, 0.02),
            (0.93, 0.0, 0.0, 0.88, 0.0, 0.6, 0.84),
            (0.06, -0.26, 0.22, 0.06, -0.08, 0.02, 0.07),
            (-0.06, 0.26, 0.22, 0.06, 0.08, 0.12, 0.07),
        ],
    },
    {
        "name": "T_Fern_Cyclosorus_03",
        "seed": 71,
        # Narrower, more pinnae — taller silhouette
        # Higher d in t2 = less compression = more pinnae
        # Smaller coefficients in t3/t4 = shorter pinnae
        "transforms": [
            (0.0, 0.0, 0.0, 0.25, 0.0, -0.14, 0.02),
...
def validate_fern(img, name):
    """Check for quality issues: top blob merging, isolated fragments."""
    alpha_arr = np.array(img.split()[3])
    issues = []

    # Check 1: Top 15% shouldn't have a solid horizontal band
    ...
    # Check 2: Count disconnected fragments
    ...
    # Check 3: Overall fill ratio (shouldn't be too dense or too sparse)
