"""Generate 3 MORE straight fern variants for Confirmed_06/07/08.
Based on proven confirmed coefficient ranges, varying shape parameters.
All straight (T2 b=0, c=0), with mild pinnae curl."""
import random
import numpy as np
...
VARIANTS = [
    {
        "name": "T_Fern_Confirmed_06",
        "desc": "Confirmed_03 variant — diamond, slightly shorter pinnae",
        "seed": 711,
        # Exact Confirmed_03 (Straight_01) but: pinnae b 0.22->0.19, angle c 0.21->0.19
        # Subtler curl, slightly narrower silhouette
        "transforms": [
            (0.0,   0.0,   0.0,   0.20,  0.0,  -0.12, 0.01),
            (0.93,  0.0,   0.0,   0.90,  0.0,   0.55, 0.84),
            (0.06, -0.19,  0.19,  0.06, -0.08,  0.02, 0.09),
            (-0.06, 0.19,  0.19,  0.06,  0.08,  0.12, 0.06),
        ],
    },
    {
        "name": "T_Fern_Confirmed_07",
        "desc": "Confirmed_04 variant — cyclosorus, slightly longer pinnae",
        "seed": 712,
        # Exact Confirmed_04 (Straight_02) but: pinnae b 0.18->0.20, same curl c=0.19
        # Wider version of cyclosorus
        "transforms": [
            (0.0,   0.0,   0.0,   0.25,  0.0,  -0.14, 0.02),
            (0.95,  0.0,   0.0,   0.93,  0.0,   0.5,  0.84),
            (0.05, -0.20,  0.19,  0.05, -0.08,  0.02, 0.07),
            (-0.05, 0.20,  0.19,  0.05,  0.08,  0.12, 0.07),
        ],
    },
    {
        "name": "T_Fern_Confirmed_08",
        "desc": "Confirmed_05 variant — oval, slightly more curl",
        "seed": 713,
        # Exact Confirmed_05 (Straight_03) but: angle c 0.23->0.25, pinnae b 0.20->0.19
        # Tighter curl, slightly shorter pinnae
        "transforms": [
            (0.0,   0.0,   0.0,   0.20,  0.0,  -0.12, 0.01),
            (0.90,  0.0,   0.0,   0.88,  0.0,   0.7,  0.80),
            (0.06, -0.19,  0.25,  0.06, -0.07,  0.02, 0.095),
            (-0.06, 0.19,  0.25,  0.06,  0.07,  0.12, 0.095),
        ],
    },
]
...
print("\nDone! Confirmed 06-08 generated.")
