                                     ...
                                     TArray<FString>& OutFailedProperties)
{
    ...
    if (UTextBlock* TextBlock = Cast<UTextBlock>(Widget))
    {
        ...
        // Handle Font property as JSON object with Size, LetterSpacing, etc.
        if (Properties->HasField(TEXT("Font")))
        {
            const TSharedPtr<FJsonObject>* FontObject;
            if (Properties->TryGetObjectField(TEXT("Font"), FontObject) && FontObject->IsValid())
            {
                FSlateFontInfo CurrentFont = TextBlock->GetFont();
                ...
                double Size = 0.0;
                if ((*FontObject)->TryGetNumberField(TEXT("Size"), Size))
                {
                ...
                if ((*FontObject)->TryGetNumberField(TEXT("LetterSpacing"), LetterSpacing))
                {
                ...
                if ((*FontObject)->TryGetNumberField(TEXT("SkewAmount"), SkewAmount))
                {
