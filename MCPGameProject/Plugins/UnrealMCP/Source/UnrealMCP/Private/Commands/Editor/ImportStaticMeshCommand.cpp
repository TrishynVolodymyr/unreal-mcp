...
FString FImportStaticMeshCommand::Execute(const FString& Parameters)
{
	...
	if (ExistingMesh)
	{
		// UE 5.7's legacy static-mesh reimport path unconditionally disables material
		...
		if (Settings.bImportMaterials)
		{
			return CreateErrorResponse(FString::Printf(
				TEXT("Same-path static-mesh reimport does not support import_materials=true in UE 5.7: %s"),
				*RequestedObjectPath));
		}
